...
def infoBarr():
	messagebox.showinfo("Обновление", "V0.1 - Создание Notepad\nV0.2 - Добавлени несколько кнопок для удобного пользования!\nV0.3 - Добавлена возможность изменения цвета фона и цвета текста, так-же обновлена иконка программы.\nV0.4 - Справка, сделана возможность добавления отзывов.")

def pravka():
	messagebox.showinfo("Справка", "Привет, это справка(не твоя из психушки) если хочешь что-то добавить в этот Notepad напишы разработчику и он это добавит!\nTelegram - http://t.me/reviewsNotepad_Bot")

def callback(event):
    webbrowser.open_new(r"http://t.me/reviewsNotepad_Bot")

root = tkinter.Tk()
root.title("Notepad v.0.4")
link = Label(root, text="Оставить отзыв", fg="blue", cursor="hand2")
link.pack()
...
menuBar.add_cascade(label="Справка", command=pravka)
